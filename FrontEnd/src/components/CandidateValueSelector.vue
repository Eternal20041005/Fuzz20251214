<template>
  <div class="candidate-value-selector">
    ...
    <div v-if="!multiple" class="single-select">
      ...
      <!-- 候选值按钮组（适用于少量选项） -->
      <div v-if="candidateValues.length > 0 && candidateValues.length <= maxButtonCount && !showCustomInput" class="button-group">
        <button
          v-for="value in candidateValues"
          :key="value"
          ...
          :class="{ 
            'is-selected': value === modelValue,
            'is-disabled': disabled
          }"
          :disabled="disabled"
          :title="getValueTooltip(value)"
        >
          {{ formatDisplayValue(value) }}
        </button>
        ...
        <button
          ...
          class="candidate-button custom-button"
          :disabled="disabled"
        >
          自定义...
        </button>
